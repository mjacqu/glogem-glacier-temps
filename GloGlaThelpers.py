...
def get_overlapping_data(validation_data, model_depth, model_temp):
    '''
    return lower and upper bounds (lb, ub) of overlapping modeled and measured data
    '''
...
def get_boundary_idxs(measured_depth, model_depth):
    '''
    get index of modeled depth above and below each measured depth value.
    '''
    ...
    if closest_value > measured_depth[1]:
...
def get_model_interpolation(lower_idx, upper_idx, model_depth, model_temp, measured_depth):
    '''
    retrieve linear interpolation between modeled depths-temperature points at measured depth
    '''
